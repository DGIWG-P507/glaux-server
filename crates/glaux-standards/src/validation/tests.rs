...
fn fixture(name: &str) -> Vec<u8> {
    fs::read(PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("corpus/fixtures").join(name)).unwrap()
}
...
fn published_corpus_expectations() {
    ...
    assert_eq!(cases.len(), 23, "required source-conflict corpus cannot disappear");
    ...
    for case in cases {
        ...
        assert_eq!(actual, case["expected_valid"].as_bool().unwrap(),
                   "source expectation failed: {}", case["id"]);
    }
...
fn fixed_encoding_selection() {
    ...
    assert_eq!(v.validate_encoding(Encoding::Json, &binary), Err(Failure::EncodingMismatch));
    assert_eq!(v.validate_encoding(Encoding::Text, &binary), Err(Failure::EncodingMismatch));
    assert_eq!(v.validate_encoding(Encoding::Json, br#"{"type":"JSONEncoding"}"#), Ok(()));
    assert_eq!(v.validate_encoding(Encoding::Text,
        br#"{"type":"TextEncoding","tokenSeparator":",","blockSeparator":"\n"}"#), Ok(()));
    assert_eq!(v.validate_encoding(Encoding::Binary, br#"{"type":"XMLEncoding"}"#),
               Err(Failure::EncodingMismatch));
    for name in ["binary-missing-byte-order.json", "binary-invalid-byte-order.json",
                 "binary-empty-members.json"] {
        assert_eq!(v.validate_encoding(Encoding::Binary, &fixture(name)), Err(Failure::Structure), "{name}");
    }
    assert_eq!(v.validate(Contract::ObservationSwe, &fixture("binary-wrapper-valid.json")), Ok(()));
    assert_eq!(v.validate(Contract::ObservationSwe, &fixture("binary-wrapper-text-encoding.json")),
               Err(Failure::Structure));
    assert_eq!(v.validate(Contract::ObservationSwe, &fixture("binary-wrapper-unresolved-component.json")),
               Ok(()), "structural success is not semantic acceptance");
}

#[test]
fn limits_and_safe_parse() {
    let v = validator();
    assert_eq!(v.validate(Contract::Quantity, &vec![b' '; MAX_BYTES + 1]), Err(Failure::Size));
    ...
    assert_eq!(parse(&serde_json::to_vec(&"x".repeat(MAX_STRING_BYTES + 1)).unwrap()), Err(Failure::String));
    assert_eq!(parse(br#"{"label":"a","l\u0061bel":"b"}"#), Err(Failure::DuplicateKey));
    for bad in [b"null null".as_slice(), b"{", b"[}", b"\xff", b"NaN", b"01", b"\"\\"] {
    ...
    assert_eq!(parse(&serde_json::to_vec(&vec![0; MAX_MEMBERS + 1]).unwrap()), Err(Failure::Members));
...
fn parser_fuzz_regressions() {
    let v = validator();
    assert_eq!(v.validate(Contract::Quantity, include_bytes!("../../../../fuzz/corpus/schema-parser/duplicate-label.json")),
               Err(Failure::DuplicateKey));
    assert_eq!(v.validate(Contract::Quantity, include_bytes!("../../../../fuzz/corpus/schema-parser/missing-label.json")),
               Err(Failure::Structure));
    for name in ["quantity-missing-label.json", "quantity-empty-label.json", "quantity-null-label.json",
                 "quantity-numeric-label.json"] {
        assert_eq!(v.validate(Contract::Quantity, &fixture(name)), Err(Failure::Structure), "{name}");
    }
}
