...
use glaux_server::http_boundary::{json_response, negotiate, HttpBoundary, Limits, Problem};
...
async fn input(State(boundary): State<HttpBoundary>, request: Request) -> Result<Response, Problem> {
...
fn decode(bytes: &[u8], head: bool) -> Wire {
    ...
    if head {
        assert!(wire.body.is_empty(), "HEAD emitted a response body: {wire:?}");
        ...
        assert_eq!(wire.body.len(), length.parse::<usize>().unwrap(), "wire body length differs");
    }
    wire
}
...
fn contract(status: u16) -> (&'static str, &'static str, &'static str) {
    match status {
        ...
        404 => ("Not Found", "not-found", "The requested resource is unavailable."),
        405 => ("Method Not Allowed", "method-not-allowed", "The method is unavailable on this route."),
        406 => ("Not Acceptable", "not-acceptable", "No offered representation is acceptable."),
        408 => ("Request Timeout", "request-timeout", "The request body did not complete within its limit."),
        413 => ("Content Too Large", "payload-too-large", "The request body exceeds its limit."),
        414 => ("URI Too Long", "uri-too-long", "The request target exceeds its limit."),
        415 => ("Unsupported Media Type", "unsupported-media-type", "The request media type or coding is unsupported."),
        431 => ("Request Header Fields Too Large", "headers-too-large", "The request headers exceed their limit."),
        500 => ("Internal Server Error", "internal", "The operation could not be completed."),
        503 => ("Service Unavailable", "unavailable", "The operation is temporarily unavailable."),
        _ => panic!("fixture contract missing"),
    }
}

fn matches_problem(wire: &Wire, status: u16) -> bool {
    let Ok(value) = serde_json::from_slice::<Value>(&wire.body) else { return false; };
    ...
    wire.status == status
        ...
        && value.get("type").and_then(Value::as_str) == Some(format!("urn:glaux:problem:{slug}").as_str())
        && value.get("title").and_then(Value::as_str) == Some(title)
        && value.get("detail").and_then(Value::as_str) == Some(detail)
        && value.get("correlation").and_then(Value::as_str).is_some_and(|correlation| {
            !correlation.is_empty() && correlation != CANARY && wire.header("x-request-id") == Some(correlation)
        })
}

fn problem(wire: &Wire, status: u16) {
    assert!(matches_problem(wire, status), "safe problem wire contract differs: expected {status}, got {wire:?}");
    let text = String::from_utf8_lossy(&wire.body);
    for forbidden in [CANARY, "SELECT password", "postgres://", "schemaPath", "stack trace"] {
        assert!(!text.contains(forbidden), "problem leaked protected details: {wire:?}");
    }
}
...
fn media(address: SocketAddr) {
    for (headers, expected) in [
        ...
        selected(&request(address, "GET", "/representation", headers, ""), expected);
    }
    for accept in ["text/xml", "application/json;q=0, application/geo+json;q=0, */*;q=1", "application/vnd.ogc.sml+json"] {
        problem(&request(address, "GET", "/representation", &format!("Accept: {accept}\r\n"), ""), 406);
    }
    for accept in ["application/json;q=1.001", "application/json;q=-1", "application/json;q=0.0000", "application/json;q=wat", "application/json;q=1;q=0", "application/json;profile=\"unterminated"] {
        problem(&request(address, "GET", "/representation", &format!("Accept: {accept}\r\n"), ""), 400);
    }
    let value = r#"{"value":17,"permittedExtension":{"unfamiliar":true}}"#;
    for media in ["application/json", "APPLICATION/JSON", "application/json;charset=utf-8", "application/json;profile=\"not-a-schema-selector\""] {
        let wire = request(address, "POST", "/json", &format!("Content-Type: {media}\r\n"), value);
        assert_eq!(wire.status, 200, "valid JSON media unexpectedly rejected: {wire:?}");
        assert_eq!(wire.json(), json!({"value":17,"permittedExtension":{"unfamiliar":true}}));
    }
    ...
    for malformed in ["", "{", "{\"x\":}", "{} trailing", "[1,]"] {
        problem(&request(address, "POST", "/json", "Content-Type: application/json\r\n", malformed), 400);
    }
    println!("HTTP boundary group passed: media-and-json-contracts");
}

fn problems(address: SocketAddr) {
    let missing = request(address, "GET", "/SyntheticPrivateSchemaCanary", "Accept: text/xml\r\nX-Request-Id: SyntheticPrivateSchemaCanary\r\n", "");
    problem(&missing, 404);
    let failed = request(address, "GET", "/private", "X-Request-Id: SyntheticPrivateSchemaCanary\r\n", "");
    problem(&failed, 500);
    assert_ne!(missing.header("x-request-id"), failed.header("x-request-id"), "correlation reused across requests");
    let method = request(address, "POST", "/representation", "", "");
    problem(&method, 405);
    let mut allowed: Vec<_> = method.header("allow").expect("405 Allow required").split(',').map(str::trim).collect();
    ...
    assert_eq!(missing_head.header("content-type"), Some("application/problem+json"));
...
fn bounds(address: SocketAddr) {
    ...
    let success = request(address, "POST", "/json", "Content-Type: application/json\r\n", &exact);
    assert_eq!(success.status, 200, "exact body limit rejected");
    assert_eq!(success.json(), json!({}));
    problem(&request(address, "POST", "/json", "Content-Type: application/json\r\n", &format!("{exact} ")), 413);
    let chunked = format!("POST /json HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\nContent-Type: application/json\r\nTransfer-Encoding: chunked\r\n\r\n80\r\n{}\r\n81\r\n{}\r\n0\r\n\r\n", " ".repeat(128), " ".repeat(129));
    problem(&exchange(address, &chunked, false), 413);
    problem(&request(address, "GET", "/representation", &format!("X-Pad: {}\r\n", "x".repeat(2500)), ""), 431);
    problem(&request(address, "GET", &format!("/{}", "a".repeat(1024)), "", ""), 414);
    ...
    let stalled = exchange(address, "POST /json HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\nContent-Type: application/json\r\nContent-Length: 16\r\n\r\n{", false);
    problem(&stalled, 408);
    assert!(started.elapsed() < Duration::from_secs(3), "body collection escaped its limit");
    let started = Instant::now();
    problem(&request(address, "GET", "/slow", "", ""), 503);
    assert!(started.elapsed() < Duration::from_secs(3), "handler escaped its deadline");
...
fn links(address: SocketAddr) {
    ...
    for headers in [
        ...
        assert_eq!(wire.json().get("href").and_then(Value::as_str), Some(expected), "configured origin changed or escaping lost");
    }
    let host = exchange(address, "GET /link HTTP/1.1\r\nHost: attacker.invalid\r\nConnection: close\r\n\r\n", false);
    assert_eq!(host.json().get("href").and_then(Value::as_str), Some(expected), "configured origin changed or escaping lost");
...
fn generated(address: SocketAddr) {
    ...
    for index in 0..128 {
        ...
        let accept = match partition {
            0 => format!("application/json;q=0.{:03}, application/geo+json;q=1", index % 1000),
            1 => format!("application/json;profile=\"seed-{index},x\";q=1, application/geo+json;q=0.5"),
            2 => format!("application/json;q=0, application/geo+json;q=0, text/x-case-{index}"),
            _ => format!("application/json;q=invalid{index}"),
        };
        let wire = request(address, "GET", "/representation", &format!("Accept: {accept}\r\n"), "");
        match partition {
    ...
    assert!(partitions.iter().all(|count| *count > 0), "generator missed a semantic partition");
    for index in 0..32 {
        let path = format!("/absent-{index}/safe%20segment?probe=escape%2F{index}%3F");
        problem(&request(address, "GET", &path, "Forwarded: host=attacker.invalid\r\n", ""), 404);
...
async fn listener(router: Router, checks: impl FnOnce(SocketAddr) + Send + 'static) {
    ...
    let outcome = done_rx.await.expect("independent client completion signal lost");
    client.join().expect("independent client thread failed outside assertions");
    stop_tx.send(()).expect("owned listener stopped before cleanup");
    tokio::time::timeout(Duration::from_secs(3), server).await.expect("owned listener cleanup timed out").unwrap();
    assert!(TcpStream::connect_timeout(&address, Duration::from_millis(100)).is_err(), "owned listener survived cleanup");
    if let Err(error) = outcome { resume_unwind(error); }
}
...
async fn proof() {
    ...
    listener(fixture(None), |address| {
        problem(&request(address, "GET", "/link", "Host-Ignored: attacker.invalid\r\n", ""), 500);
    }).await;
...
fn main() {
    ...
    tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap().block_on(async {
        tokio::time::timeout(Duration::from_secs(30), proof()).await.expect("bounded HTTP boundary proof timed out");
    });
}
