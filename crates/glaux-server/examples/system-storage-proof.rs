...
fn record(suffix: &str, uid: &str, sources: Vec<SourceIdentity>, parent: Option<LocalId>) -> SystemRecord {
    SystemRecord {
        id: id(suffix),
...
fn parent() -> SystemRecord {
    record(
        "3901", "urn:glaux:fixture:system:P",
        vec![source("authority-A", "platform-7"), source("alternate-authority", "P-ALIAS")],
        None,
    )
}
...
fn first_child() -> SystemRecord {
    record(
        "3902", "urn:glaux:fixture:system:A",
        vec![source("authority-A", "sensor-1"), source("authority-A", "sensor-1-alias")],
        Some(id("3901")),
    )
}
...
fn expected(records: &[&SystemRecord], identity_only: bool) -> Snapshot {
    let mut result = Snapshot {
        identities: Vec::new(), systems: Vec::new(), sources: Vec::new(), parents: Vec::new(),
    };
    for record in records {
        let local_id = record.id.to_string();
        result.identities.push((local_id.clone(), "system".into(), record.uid.to_string()));
        result.systems.push((local_id.clone(), record.label.clone()));
        for alias in &record.sources {
            result.sources.push((local_id.clone(), alias.authority().to_string(), alias.identifier().to_string()));
        }
    ...
    if identity_only {
        result.identities.push((id("39f0").to_string(), "system".into(), "urn:glaux:fixture:identity-only".into()));
    }
...
async fn snapshot(connection: &mut PgConnection, with_parents: bool) -> Snapshot {
    ...
    Snapshot { identities, systems, sources, parents }
}
...
fn assert_record(actual: &SystemRecord, expected: &SystemRecord) {
    ...
    let mut expected_sources: Vec<_> = expected.sources.iter().map(|alias| {
        (alias.authority().as_str(), alias.identifier().as_str())
    }).collect();
    expected_sources.sort();
    let actual_sources: Vec<_> = actual.sources.iter().map(|alias| {
        (alias.authority().as_str(), alias.identifier().as_str())
    }).collect();
    assert_eq!(actual_sources, expected_sources, "complete canonical source-pair order");
}

async fn assert_lookup(connection: &mut PgConnection, expected: &SystemRecord) {
    let by_id = SystemRepository::get(&mut *connection, expected.id).await.unwrap().expect("known ID");
    assert_record(&by_id, expected);
    let by_uid = SystemRepository::find_uid(&mut *connection, &expected.uid).await.unwrap().expect("known UID");
    assert_record(&by_uid, expected);
    for alias in &expected.sources {
        let by_source = SystemRepository::find_source(&mut *connection, alias).await.unwrap().expect("known source pair");
        assert_record(&by_source, expected);
    }
}

async fn expect_conflict(connection: &mut PgConnection, attempted: &SystemRecord, unchanged: &Snapshot) {
    assert_eq!(&snapshot(connection, true).await, unchanged);
    let result = SystemRepository::create(&mut *connection, attempted).await;
    assert!(matches!(result, Err(StorageError::Conflict)), "expected identity conflict, got {result:?}");
    assert_eq!(&snapshot(connection, true).await, unchanged, "conflict must leave every table unchanged");
}

async fn expect_invalid_parent(connection: &mut PgConnection, attempted: &SystemRecord, unchanged: &Snapshot) {
    assert_eq!(&snapshot(connection, true).await, unchanged);
    let result = SystemRepository::create(&mut *connection, attempted).await;
    assert!(matches!(result, Err(StorageError::InvalidAssociation)), "expected invalid typed endpoint, got {result:?}");
    assert_eq!(&snapshot(connection, true).await, unchanged, "parent failure must roll back the whole create");
}

async fn proof() {
    let mut connection = PgConnection::connect(DSN).await.expect("owned disposable PostgreSQL is required");
    let location: (String, String, String) = sqlx::query_as(
        "SELECT current_database(), current_user, inet_server_addr()::text",
    ).fetch_one(&mut connection).await.unwrap();
    assert_eq!(location, ("glaux_harness_test".into(), "postgres".into(), "127.0.0.1".into()));
    sqlx::query("SET statement_timeout = '5s'").execute(&mut connection).await.unwrap();
    sqlx::query("SET lock_timeout = '1s'").execute(&mut connection).await.unwrap();
    ...
    let absent_parent: (Option<String>,) = sqlx::query_as("SELECT to_regclass('public.system_parent')::text")
        .fetch_one(&mut connection).await.unwrap();
    ...
    assert_eq!(snapshot(&mut connection, false).await, predecessor_rows);
    migrate(&mut connection).await.expect("explicit packaged upgrade");
    check_schema(&mut connection).await.expect("latest compatible schema");
    ...
    assert!(SystemRepository::get(&mut connection, id("39ff")).await.unwrap().is_none());
    let unknown_uid: Uid = "urn:glaux:fixture:unknown".parse().unwrap();
    assert!(SystemRepository::find_uid(&mut connection, &unknown_uid).await.unwrap().is_none());
    assert!(SystemRepository::find_source(&mut connection, &source("authority-C", "sensor-1")).await.unwrap().is_none());
    println!("System storage group passed: exact-identity-and-lookups");

    let duplicate_id = record("3901", "urn:glaux:rejected:duplicate-id", vec![source("attempt", "duplicate-id")], None);
    expect_conflict(&mut connection, &duplicate_id, &initial).await;
    let duplicate_uid = record("3941", p.uid.as_str(), vec![source("attempt", "duplicate-uid")], Some(p.id));
    expect_conflict(&mut connection, &duplicate_uid, &initial).await;
    let duplicate_source = record(
        "3942", "urn:glaux:rejected:duplicate-source",
        vec![source("aaa-new-authority", "fresh-only-then-conflict"), source("authority-A", "sensor-1")],
        Some(p.id),
    );
    ...
    let l1 = record("3904", &long_uid, vec![source(&long_authority, &long_identifier)], Some(p.id));
    ...
    assert_eq!(pristine_ledger.iter().map(|row| row.0).collect::<Vec<_>>(), vec![1, 2, 3]);
    migrate(&mut connection).await.expect("reapplying packaged migrations is nondestructive");
    ...
    for mutation in [
        ...
        assert_eq!(sqlx::query(mutation).execute(&mut connection).await.unwrap().rows_affected(), 1);
        let polluted = ledger(&mut connection).await;
        assert_ne!(polluted, pristine_ledger, "corruption fixture must alter the ledger");
        assert!(matches!(check_schema(&mut connection).await, Err(StorageError::IncompatibleSchema)), "reject dirty, changed-checksum and unknown-version schemas");
        assert_eq!(ledger(&mut connection).await, polluted, "compatibility check must be read-only");
        assert_eq!(snapshot(&mut connection, true).await, complete);
    ...
    connection.close().await.expect("explicit proof connection close");
    println!("Required System storage proof passed: 7 groups.");
}

fn main() {
    assert_eq!(std::env::args_os().len(), 1, "no target or test-selection overrides are accepted");
    let runtime = tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap();
    runtime.block_on(async {
        tokio::time::timeout(Duration::from_secs(90), proof()).await.expect("System storage proof deadline exceeded");
    });
}
