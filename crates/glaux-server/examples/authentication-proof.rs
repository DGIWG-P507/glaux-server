...
use glaux_server::{
    authentication::{Authenticator, CallerContext, CallerKind, Clock, DevelopmentConfig, JwtConfig},
...
fn fixture(auth: Authenticator, count: Arc<AtomicUsize>) -> Router {
    ...
    HttpBoundary::new(None, Limits::default()).unwrap().router(auth.protect(routes))
}
...
fn bearer(fixtures: &Value, name: &str) -> String {
    format!("Authorization: Bearer {}\r\n", fixtures["tokens"][name].as_str().expect("named independent token required"))
}
...
fn oracle_controls() {
    ...
    for bad in [json!(null), json!(17), json!("attacker-subject")] {
        ...
        assert!(!matches_identity(&wrong), "bad identity field escaped independent oracle");
    }
    ...
    assert!(!matches_identity(&wrong), "missing identity escaped independent oracle");
...
fn accepted(address: SocketAddr, headers: &str, count: &AtomicUsize) {
    ...
    assert!(matches_identity(&wire), "valid signed access token did not produce exact verified caller: {wire:?}");
    assert_eq!(count.load(Ordering::SeqCst), before + 1);
}

fn denied(address: SocketAddr, path: &str, headers: &str, status: u16, challenge: Option<&str>, count: &AtomicUsize) {
    let before = count.load(Ordering::SeqCst);
    let wire = request(address, path, headers);
    assert_eq!(wire.status, status, "credential rejection status differs: {wire:?}");
    assert_eq!(wire.header("www-authenticate"), challenge);
    assert_eq!(wire.header("content-type"), Some("application/problem+json"));
    ...
    let (slug, title, detail) = match status {
        ...
        403 => ("forbidden", "Forbidden", "The credential lacks required scope."),
    ...
    assert!(wire.header("x-request-id").is_some_and(|value| !value.is_empty()));
    let body = String::from_utf8_lossy(&wire.body);
    for secret in [CANARY, "fixture-alice", "fixture-client", "group-a", "BEGIN PRIVATE KEY", "stack trace"] {
        assert!(!body.contains(secret), "credential problem disclosed private context");
    }
    assert_eq!(count.load(Ordering::SeqCst), before, "rejected credential reached protected handler");
}

fn verified(address: SocketAddr, fixtures: &Value, count: &AtomicUsize) {
    for name in ["valid", "valid-media-type", "valid-case-type", "valid-aud-array", "valid-extension"] {
        accepted(address, &bearer(fixtures, name), count);
    }
    accepted(address, &bearer(fixtures, "valid").replace("Bearer", "bEaReR"), count);
    accepted(address, &(bearer(fixtures, "valid") + "X-User: attacker\r\nX-Groups: administrators\r\nForwarded: for=192.0.2.1\r\n"), count);
...
fn rejections(address: SocketAddr, fixtures: &Value, count: &AtomicUsize) {
    for name in [
        "wrong-audience", "wrong-issuer", "bad-signature", "id-token", "missing-type",
        "unknown-algorithm", "unknown-key", "unsupported-critical", "unencoded-payload",
        "unsigned", "symmetric-confusion", "duplicate-claim", "duplicate-header",
        "audience-empty", "audience-mixed", "subject-type", "expiry-type", "scope-type",
        "groups-type", "groups-mixed", "missing-iss", "missing-sub", "missing-aud",
        "missing-exp", "missing-iat", "missing-client_id", "missing-jti",
    ] {
        // The first case is the controlled audience-bypass fault's exact target.
        let before = count.load(Ordering::SeqCst);
        let wire = request(address, "/who", &bearer(fixtures, name));
        assert_eq!(wire.status, 401, "invalid access-token case accepted: {name}");
        assert_eq!(count.load(Ordering::SeqCst), before, "invalid token reached handler: {name}");
        denied(address, "/who", &bearer(fixtures, name), 401, Some("Bearer error=\"invalid_token\""), count);
    }
...
fn validity(address: SocketAddr, fixtures: &Value, count: &AtomicUsize, clock: &ControlledClock) {
    for name in ["expired", "future-not-before", "future-issued", "expiry-before-issued",
        "fraction-exp-equal", "fraction-exp-before", "fraction-nbf-after", "fraction-iat-after"] {
        denied(address, "/who", &bearer(fixtures, name), 401, Some("Bearer error=\"invalid_token\""), count);
    }
    for name in ["fraction-exp-after", "fraction-nbf-equal", "fraction-iat-equal"] {
        accepted(address, &bearer(fixtures, name), count);
    }
    denied(address, "/who", &bearer(fixtures, "missing-scope"), 403, Some("Bearer error=\"insufficient_scope\""), count);
    *clock.0.lock().unwrap() = None;
    denied(address, "/who", &bearer(fixtures, "valid"), 503, None, count);
...
fn framing(address: SocketAddr, fixtures: &Value, count: &AtomicUsize) {
    ...
    denied(address, "/who?access_token=ignored", "X-User: administrator\r\n", 401, Some("Bearer"), count);
    denied(address, "/who", "Authorization: Basic SyntheticCredentialSecretCanary\r\n", 401, Some("Bearer"), count);
    for headers in [
        "Authorization: Bearer\r\n".to_owned(),
        "Authorization: Bearer a b\r\n".to_owned(),
        bearer(fixtures, "valid") + &bearer(fixtures, "valid"),
    ] {
        denied(address, "/who", &headers, 400, Some("Bearer error=\"invalid_request\""), count);
    }
    for token in ["one", "a.b.c", "a.b.c.d", "%%%.e30.invalid", CANARY] {
        denied(address, "/who", &format!("Authorization: Bearer {token}\r\n"), 401, Some("Bearer error=\"invalid_token\""), count);
    }
...
fn config_controls(fixtures: &Value, clock: Arc<ControlledClock>) {
    ...
    let auth = Authenticator::development(development_config(), "127.0.0.1:8080".parse().unwrap()).unwrap();
    assert!(auth.authenticate(&HeaderMap::new(), None).is_err());
    assert!(auth.authenticate(&HeaderMap::new(), Some("192.0.2.1:1234".parse().unwrap())).is_err());
    assert!(auth.authenticate(&HeaderMap::new(), Some("127.0.0.1:1234".parse().unwrap())).is_ok());
    assert!(auth.authenticate(&HeaderMap::new(), Some("[::1]:1234".parse().unwrap())).is_ok());
}
...
async fn listener(router: Router, checks: impl FnOnce(SocketAddr) + Send + 'static) {
    ...
    tokio::time::timeout(Duration::from_secs(3), server).await.expect("listener cleanup timed out").unwrap();
...
async fn proof(fixtures: Value) {
    ...
    let auth = Authenticator::jwt(jwt_config(&fixtures), clock.clone()).expect("valid public-key configuration rejected");
    ...
    let dev = Authenticator::development(development_config(), "127.0.0.1:8080".parse().unwrap()).unwrap();
    let dev_count = count.clone();
    let valid_bearer = bearer(&fixtures, "valid");
    listener(fixture(dev, count.clone()), move |address| {
        for headers in ["", "X-User: attacker\r\nX-Groups: administrators\r\nForwarded: for=192.0.2.1\r\n"] {
        ...
        for headers in [valid_bearer.as_str(), "Authorization: Bearer SyntheticCredentialSecretCanary\r\n"] {
            denied(address, "/who", headers, 401, Some("Bearer error=\"invalid_token\""), &dev_count);
        }
    }).await;
    let disabled_count = count.clone();
    listener(fixture(Authenticator::disabled(), count.clone()), move |address| {
...
fn main() {
    ...
    tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap().block_on(async {
        tokio::time::timeout(Duration::from_secs(30), proof(fixtures)).await.expect("bounded authentication proof timed out");
    });
}
