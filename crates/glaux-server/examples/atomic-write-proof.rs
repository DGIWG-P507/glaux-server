...
async fn assert_accepted(connection: &mut PgConnection) {
    ...
    .await
    .unwrap();
    assert_eq!(system, (id("0102").to_string(), "system".to_owned(),
        "urn:glaux:test:atomic-child".to_owned(), "Atomic child".to_owned(), id("0101").to_string()));
    ...
    assert_eq!(sources, vec![
        (id("0102").to_string(), "urn:glaux:test:source-a".to_owned(), "upstream-17".to_owned()),
        (id("0102").to_string(), "urn:glaux:test:source-b".to_owned(), "upstream-29".to_owned()),
    ]);
    ...
    .await
    .unwrap();
    assert_eq!(artifact, (id("0201").to_string(), "application/json".to_owned(), BYTES.to_vec(), DIGEST.to_owned()));
    ...
    for (column, expected) in [
        ("id", id("0301").to_string()), ("system_id", id("0102").to_string()),
        ("artifact_id", id("0201").to_string()), ("semantic_fraction", "0.0000011".to_owned()),
        ("semantic_source", SEMANTIC.to_owned()), ("receipt_fraction", "0.12345678901234567890".to_owned()),
        ("receipt_source", RECEIPT.to_owned()),
    ] {
        assert_eq!(revision.get::<String, _>(column), expected, "revision {column}");
    }
    assert_eq!(revision.get::<i64, _>("semantic_civil_second"), 0);
    assert_eq!(revision.get::<i64, _>("receipt_civil_second"), 1_483_228_800);
    ...
    for (column, expected) in [
        ("id", id("0401").to_string()), ("actor", "fixture-actor".to_owned()),
        ("source", "fixture-source".to_owned()), ("operation", "system.create".to_owned()),
        ("target_id", id("0102").to_string()), ("revision_id", id("0301").to_string()),
        ("time_fraction", "0.12345678901234567890".to_owned()), ("time_source", RECEIPT.to_owned()),
        ("outcome", "accepted".to_owned()), ("correlation", "fixture-correlation-15".to_owned()),
    ] {
    ...
    assert_eq!(work.unwrap(), (id("0501").to_string(), id("0102").to_string(), id("0301").to_string(),
        id("0201").to_string(), id("0401").to_string(), "system.created".to_owned(), "accepted".to_owned()));
    ...
    .await
    .unwrap();
    assert_eq!(linked, (id("0102").to_string(), id("0301").to_string(), RECEIPT.to_owned(), BYTES.to_vec(), DIGEST.to_owned()));
}
...
async fn accepted(connection: &mut PgConnection) {
    ...
    assert!(matches!(create_system(connection, &fixture()).await, Err(StorageError::Conflict)));
    assert_eq!(snapshot(connection).await, before, "duplicate attempt changed committed context");
    for statement in [
...
async fn insert_failures(connection: &mut PgConnection) {
    ...
    let cases = [
        ...
        ("first-alias", "CREATE TRIGGER fixture_fail AFTER INSERT ON public.source_identity FOR EACH ROW EXECUTE FUNCTION public.fixture_write_failure()", "DROP TRIGGER fixture_fail ON public.source_identity"),
        ("second-alias", "CREATE TRIGGER fixture_fail AFTER INSERT ON public.source_identity FOR EACH ROW EXECUTE FUNCTION public.fixture_second_alias_failure()", "DROP TRIGGER fixture_fail ON public.source_identity"),
        ("parent-guard", "CREATE TRIGGER fixture_fail AFTER UPDATE ON public.system_parent_write_guard FOR EACH ROW EXECUTE FUNCTION public.fixture_write_failure()", "DROP TRIGGER fixture_fail ON public.system_parent_write_guard"),
        ("parent", "CREATE TRIGGER fixture_fail AFTER INSERT ON public.system_parent FOR EACH ROW EXECUTE FUNCTION public.fixture_write_failure()", "DROP TRIGGER fixture_fail ON public.system_parent"),
        ("artifact", "CREATE TRIGGER fixture_fail AFTER INSERT ON public.source_artifact FOR EACH ROW EXECUTE FUNCTION public.fixture_write_failure()", "DROP TRIGGER fixture_fail ON public.source_artifact"),
        ("revision", "CREATE TRIGGER fixture_fail AFTER INSERT ON public.system_revision FOR EACH ROW EXECUTE FUNCTION public.fixture_write_failure()", "DROP TRIGGER fixture_fail ON public.system_revision"),
        ("audit", "CREATE TRIGGER fixture_fail AFTER INSERT ON public.server_audit FOR EACH ROW EXECUTE FUNCTION public.fixture_write_failure()", "DROP TRIGGER fixture_fail ON public.server_audit"),
        ("outgoing", "CREATE TRIGGER fixture_fail AFTER INSERT ON public.outgoing_work FOR EACH ROW EXECUTE FUNCTION public.fixture_write_failure()", "DROP TRIGGER fixture_fail ON public.outgoing_work"),
        ("commit", "CREATE CONSTRAINT TRIGGER fixture_fail AFTER INSERT ON public.outgoing_work DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION public.fixture_write_failure()", "DROP TRIGGER fixture_fail ON public.outgoing_work"),
    ];
    for (name, install, remove) in cases {
        reset(connection).await;
        let before = snapshot(connection).await;
        execute(connection, install).await;
        let error = create_system(connection, &fixture()).await.err().expect("injected write must fail");
        ...
        assert_eq!(snapshot(connection).await, before, "atomic rollback failed at {name}");
        assert!(!connection.is_in_transaction(), "failed transaction remained open at {name}");
...
async fn invalid_context(connection: &mut PgConnection) {
    ...
    for case in 0..11 {
        ...
        assert!(matches!(create_system(connection, &request).await, Err(StorageError::InvalidInput)), "invalid context case {case}");
        assert_eq!(snapshot(connection).await, before, "invalid context changed state case {case}");
    }
    let mut transaction = connection.begin().await.unwrap();
    assert!(matches!(create_system(&mut transaction, &fixture()).await, Err(StorageError::InvalidInput)));
    assert!(matches!(record_denied_system_create(&mut transaction, &denial()).await, Err(StorageError::InvalidInput)));
    assert_eq!(snapshot(&mut transaction).await, before, "ambient transaction was mutated");
    transaction.rollback().await.unwrap();
    assert_eq!(snapshot(connection).await, before);
    let mut invalid_denial = denial();
    invalid_denial.audit.correlation = String::new();
    assert!(matches!(record_denied_system_create(connection, &invalid_denial).await,
    ...
    let actual_source: Option<String> = sqlx::query_scalar("SELECT source FROM public.server_audit")
        .fetch_one(connection).await.unwrap();
    assert!(actual_source.is_none(), "missing verified source must stay unknown");
...
async fn visibility(connection: &mut PgConnection) {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            ...
            ).bind(writer_pid).fetch_one(&mut *connection).await.unwrap();
            if waiting { break; }
            assert!(!task.is_finished(), "writer ended before reaching its explicit visibility barrier");
            tokio::task::yield_now().await;
        }
    }).await.expect("writer did not reach advisory barrier within bound");
    assert_eq!(snapshot(connection).await, before, "uncommitted application facts became visible");
    ...
    execute(connection, "DROP TRIGGER fixture_visibility ON public.outgoing_work").await;
    assert_accepted(connection).await;
...
async fn denied(connection: &mut PgConnection) {
    ...
    for column in ["actor", "source", "target_id", "revision_id"] {
        assert!(row.get::<Option<String>, _>(column).is_none(), "denial invented {column}");
    }
    for (column, expected) in [
        ("operation", "system.create"), ("outcome", "denied"),
        ("time_fraction", "0.12345678901234567890"), ("time_source", RECEIPT),
    ...
    execute(connection, "ALTER TABLE public.server_audit DISABLE TRIGGER server_audit_immutable").await;
    execute(connection, "DELETE FROM public.server_audit").await;
    execute(connection, "ALTER TABLE public.server_audit ENABLE TRIGGER server_audit_immutable").await;
    assert_eq!(snapshot(connection).await, before, "denial mutated non-audit state");
    let mut known = denial();
    known.target = Some(id("0199"));
    known.audit = context();
    record_denied_system_create(connection, &known).await.unwrap();
    let known_row: (Option<String>, Option<String>, Option<String>, Option<String>, String) = sqlx::query_as(
        "SELECT actor,source,target_id::text,revision_id::text,outcome FROM public.server_audit",
    ).fetch_one(&mut *connection).await.unwrap();
    assert_eq!(known_row, (Some("fixture-actor".to_owned()), Some("fixture-source".to_owned()), Some(id("0199").to_string()), None, "denied".to_owned()));
...
async fn permissions(connection: &mut PgConnection) {
    ...
    for statement in [
        ...
        let error = sqlx::query(statement).execute(&mut serving).await.unwrap_err();
        assert_eq!(error.as_database_error().and_then(|e| e.code()).as_deref(), Some("42501"));
        assert_eq!(snapshot(connection).await, before, "serving role changed earlier audit");
    }
    ...
    for statement in [
        ...
        assert_eq!(snapshot(connection).await, before, "immutable audit guard failed");
    }
    ...
    let deletion = sqlx::query("DELETE FROM public.system_identity WHERE id='01890f20-7b5a-7cc3-98c4-dc0c0c070102'")
    ...
    assert_eq!(audit_fks, 0, "audit retention must not depend on resource existence");
...
async fn proof() {
    ...
    let identity: (String, String, String) = sqlx::query_as(
        "SELECT current_database(),current_user::text,inet_server_addr()::text",
    ).fetch_one(&mut connection).await.unwrap();
    assert_eq!(identity, ("glaux_harness_test".to_owned(), "postgres".to_owned(), "127.0.0.1".to_owned()));
    execute(&mut connection, "SET statement_timeout=5000").await;
    execute(&mut connection, "SET lock_timeout=1000").await;
    packaged_migrations().run_to(5, &mut connection).await.unwrap();
    assert!(matches!(SystemRepository::create(&mut connection, &parent()).await,
    ...
    assert_eq!(SystemRepository::get(&mut connection, id("0101")).await.unwrap(), Some(parent()));
    let before = snapshot(&mut connection).await;
    migrate(&mut connection).await.unwrap();
    assert_eq!(snapshot(&mut connection).await, before, "migration reapply changed data");
