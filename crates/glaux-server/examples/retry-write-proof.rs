...
fn id(value: u16) -> LocalId {
    format!("01890f20-7b5a-7cc3-98c4-dc0c0c09{value:04x}").parse().unwrap()
}
...
fn input(generation: u16, logical: u16) -> CreateSystem {
    CreateSystem {
        system: SystemRecord {
            ...
            sources: vec![
                SourceIdentity::new("urn:glaux:retry:a".parse().unwrap(), format!("a-{logical}").parse().unwrap()),
                SourceIdentity::new("urn:glaux:retry:b".parse().unwrap(), format!("b-{logical}").parse().unwrap()),
            ],
            parent: None,
        },
...
fn key() -> RetryKey {
    RetryKey { key: "opaque-retry-key".to_owned(), retention_seconds: 3600 }
}
...
async fn execute(connection: &mut PgConnection, sql: &str) {
    ...
    sqlx::query(AssertSqlSafe(sql)).execute(connection).await.unwrap();
}

fn passed(name: &str) { println!("Retry write group passed: {name}"); }
...
async fn assert_records(connection: &mut PgConnection, generations: &[u16]) {
    for (table, offset) in [("resource_identity",1000), ("source_artifact",2000), ("system_revision",3000), ("server_audit",4000), ("outgoing_work",5000)] {
        let actual: Vec<String> = sqlx::query_scalar(AssertSqlSafe(format!("SELECT id::text FROM public.{table} ORDER BY id")))
            .fetch_all(&mut *connection).await.unwrap();
        let mut expected: Vec<String> = generations.iter().map(|n| id(offset + n).to_string()).collect();
        expected.sort();
    ...
    for generation in generations {
        ...
        for (column, value) in [
            ("system_id",id(1000 + generation).to_string()), ("artifact_id",id(2000 + generation).to_string()),
            ("semantic_source",TIME.to_owned()), ("receipt_source",TIME.to_owned()),
            ("media_type","application/octet-stream".to_owned()), ("actor","retry-actor".to_owned()),
            ("source","retry-source".to_owned()), ("operation","system.create".to_owned()),
            ("target_id",id(1000 + generation).to_string()), ("revision_id",id(3000 + generation).to_string()),
            ("outcome","accepted".to_owned()), ("correlation",format!("request-{generation}")),
            ("event",id(5000 + generation).to_string()), ("work_system",id(1000 + generation).to_string()),
            ("work_revision",id(3000 + generation).to_string()), ("work_artifact",id(2000 + generation).to_string()),
            ("work_audit",id(4000 + generation).to_string()), ("kind","system.created".to_owned()),
            ("head_revision",id(3000 + generation).to_string()), ("head_artifact",id(2000 + generation).to_string()),
        ] { assert_eq!(row.get::<String,_>(column), value, "exact persisted {column}"); }
        assert_eq!(row.get::<Vec<u8>,_>("bytes"), b"independent-original-bytes");
    }
}
...
async fn replay(connection: &mut PgConnection) {
    ...
    create_system_with_retry(&mut lost,&input(1,1),Some(&key()), |_| true).await.unwrap();
    ...
    assert!(matches!(result,Ok(value) if value==expected(1)), "identical retry did not return original receipt");
    assert_eq!(snapshot(connection).await,before,"replay changed committed state");
    let mut reconnected = PgConnection::connect(DSN).await.unwrap();
    assert_eq!(create_system_with_retry(&mut reconnected,&input(4,1),Some(&key()), |_| true).await.unwrap(),expected(1));
    ...
    assert_eq!(binding,("retry-actor".to_owned(),"retry-source".to_owned(),"system.create".to_owned(),String::new(),key().key,
        id(1001).to_string(),id(3001).to_string(),id(2001).to_string(),id(4001).to_string(),true));
    migrate(connection).await.unwrap();
    assert_eq!(snapshot(connection).await,before,"migration reapply changed receipt");
    passed("original-outcome-generated-ids-lost-response");
}

async fn content(connection: &mut PgConnection) {
    reset(connection).await;
    create_system_with_retry(connection,&input(1,1),Some(&key()), |_| true).await.unwrap();
    let before = snapshot(connection).await;
    for case in 0..9 {
        ...
        match case {
            ...
            2 => { changed.system.sources.pop(); },
            3 => changed.system.sources[0] = SourceIdentity::new("urn:glaux:changed".parse().unwrap(),"a-1".parse().unwrap()),
            4 => changed.system.sources[0] = SourceIdentity::new("urn:glaux:retry:a".parse().unwrap(),"changed".parse().unwrap()),
            ...
            8 => changed.revision.semantic_time = Some("2017-01-01T00:00:00.12345678901234567890Z".parse().unwrap()),
            _ => unreachable!(),
        }
        assert!(matches!(create_system_with_retry(connection,&changed,Some(&key()), |_| true).await,Err(StorageError::Conflict)),
            "changed retry intent was not rejected: case {case}");
        assert_eq!(snapshot(connection).await,before,"content conflict mutated state: {case}");
    }
...
async fn scopes(connection: &mut PgConnection) {
    ...
    assert!(matches!(create_system_with_retry(connection,&input(2,1),Some(&key()), |r| {
        authorized_target=Some(*r); false
    }).await,Err(StorageError::Denied)),"saved receipt disclosed before reauthorization");
    assert_eq!(authorized_target,Some(expected(1)),"reauthorization used new generated target");
    assert_eq!(snapshot(connection).await,before);
    let mut changed = input(2,1);
    changed.artifact.bytes=b"denied changed intent".to_vec();
    assert!(matches!(create_system_with_retry(connection,&changed,Some(&key()), |_| false).await,Err(StorageError::Denied)),
    ...
    for case in 0..3 {
        ...
        assert_eq!(create_system_with_retry(connection,&request,Some(&key()), |_| true).await.unwrap(),expected(generation),
            "different caller/source reused another scope's receipt");
        let before = snapshot(connection).await;
        assert_eq!(create_system_with_retry(connection,&request,Some(&key()), |_| true).await.unwrap(),expected(generation));
        assert_eq!(snapshot(connection).await,before);
    }
    let parent = SystemRecord {id:id(999),uid:"urn:glaux:retry:parent".parse().unwrap(),label:"Parent".to_owned(),sources:vec![],parent:None};
    SystemRepository::create(connection,&parent).await.unwrap();
    let mut child = input(5,5);
    child.system.parent=Some(parent.id);
    assert_eq!(create_system_with_retry(connection,&child,Some(&key()), |_| true).await.unwrap(),expected(5));
    let target: String = sqlx::query_scalar("SELECT target FROM public.system_create_retry WHERE system_id=$1::text::uuid")
        .bind(id(1005).to_string()).fetch_one(&mut *connection).await.unwrap();
    assert_eq!(target,parent.id.to_string());
    let before = snapshot(connection).await;
    assert!(matches!(create_system_with_retry(connection,&input(6,6),Some(&key()), |_| false).await,Err(StorageError::Denied)));
    assert_eq!(snapshot(connection).await,before);
    let request = input(7,1);
    let update = UpdateSystem {system_id:id(1001),label:"Updated independently".to_owned(),artifact:request.artifact,
        revision:SystemRevision {system_id:id(1001),..request.revision},audit_id:request.audit_id,event_id:request.event_id,audit:request.audit,expected_revision:None};
    assert_eq!(update_system(connection,&update).await.unwrap().revision_id,id(3007).to_string().parse().unwrap());
    assert_eq!(create_system_with_retry(connection,&input(8,1),Some(&key()), |_| true).await.unwrap(),expected(1),
...
async fn expiry(connection: &mut PgConnection) {
    ...
    create_system_with_retry(connection,&input(1,1),Some(&key()), |_| true).await.unwrap();
    let before = snapshot(connection).await;
    let extended = RetryKey {retention_seconds:7200,..key()};
    assert_eq!(create_system_with_retry(connection,&input(2,1),Some(&extended), |_| true).await.unwrap(),expected(1));
    assert_eq!(snapshot(connection).await,before,"replay extended existing retention");
    ...
    let expired: bool = sqlx::query_scalar("SELECT expires_at<clock_timestamp() FROM public.system_create_retry")
        .fetch_one(&mut *connection).await.unwrap();
    assert!(expired,"expiry fixture does not precede actual database clock");
    assert_eq!(create_system_with_retry(connection,&input(2,2),Some(&key()), |_| true).await.unwrap(),expected(2));
    assert_records(connection,&[1,2]).await;
    let receipt_ids: Vec<String> = sqlx::query_scalar("SELECT system_id::text FROM public.system_create_retry")
        .fetch_all(&mut *connection).await.unwrap();
    assert_eq!(receipt_ids,vec![id(1002).to_string()]);
    reset(connection).await;
    for generation in [1,2] {
        assert_eq!(create_system_with_retry(connection,&input(generation,generation),None, |_| true).await.unwrap(),expected(generation));
    }
    let count: i64 = sqlx::query_scalar("SELECT count(*) FROM public.system_create_retry").fetch_one(&mut *connection).await.unwrap();
    assert_eq!(count,0,"optional key became mandatory or manufactured");
    let before = snapshot(connection).await;
    let mut duplicate_alias = input(3,3);
    duplicate_alias.system.sources.push(SourceIdentity::new("urn:glaux:retry:a".parse().unwrap(),"a-3".parse().unwrap()));
    assert!(matches!(create_system_with_retry(connection,&duplicate_alias,Some(&key()), |_| true).await,Err(StorageError::InvalidInput)),
        "invalid duplicate aliases were normalized into accepted intent");
    assert_eq!(snapshot(connection).await,before);
    for retry in [RetryKey{key:String::new(),retention_seconds:3600},RetryKey{key:"x".repeat(257),retention_seconds:3600},RetryKey{key:"bad\nkey".to_owned(),retention_seconds:3600},RetryKey{key:"valid".to_owned(),retention_seconds:0}] {
        assert!(matches!(create_system_with_retry(connection,&input(3,3),Some(&retry), |_| true).await,Err(StorageError::InvalidInput)));
        assert_eq!(snapshot(connection).await,before);
    }
    let mut transaction = connection.begin().await.unwrap();
    assert!(matches!(create_system_with_retry(&mut transaction,&input(3,3),Some(&key()), |_| true).await,Err(StorageError::InvalidInput)));
...
async fn rollback_and_constraints(connection: &mut PgConnection) {
    ...
    for deferred in [false,true] {
        ...
        let suffix = if deferred {"DEFERRABLE INITIALLY DEFERRED "} else {""};
        ...
        let error = create_system_with_retry(connection,&input(1,1),Some(&key()), |_| true).await.unwrap_err();
        assert!(matches!(&error,StorageError::Database(e) if e.as_database_error().and_then(|e|e.code()).as_deref()==Some("P0001")),"wrong receipt rollback failure: {error}");
        execute(connection,"DROP TRIGGER retry_fail ON public.system_create_retry").await;
        assert_eq!(snapshot(connection).await,before,"receipt or commit failure left partial state");
        assert!(!connection.is_in_transaction());
        println!("Retry rollback boundary passed: {}",if deferred {"commit"} else {"receipt"});
    }
    create_system_with_retry(connection,&input(1,1),Some(&key()), |_| true).await.unwrap();
    let before=snapshot(connection).await;
    for (sql,code) in [
        ("UPDATE public.system_create_retry SET event_id='01890f20-7b5a-7cc3-98c4-dc0c0c09138a'","23503"),
        ("UPDATE public.system_create_retry SET operation='system.update'","23514"),
        ("UPDATE public.system_create_retry SET digest=decode('00','hex')","23514"),
    ] {
        let error=sqlx::query(sql).execute(&mut *connection).await.unwrap_err();
        assert_eq!(error.as_database_error().and_then(|e|e.code()).as_deref(),Some(code));
        assert_eq!(snapshot(connection).await,before);
    }
...
async fn concurrent(connection: &mut PgConnection,first:u16,conflicting:bool) {
    ...
    for writer in [&mut winner,&mut waiter] {
        ...
        execute(writer,"SET default_transaction_isolation='repeatable read'").await;
    }
    let winner_pid:i32=sqlx::query_scalar("SELECT pg_backend_pid()").fetch_one(&mut winner).await.unwrap();
    let waiter_pid:i32=sqlx::query_scalar("SELECT pg_backend_pid()").fetch_one(&mut waiter).await.unwrap();
    let winner_task=tokio::spawn(async move {create_system_with_retry(&mut winner,&input(first,1),Some(&key()), |_| true).await});
    wait_for_lock(connection,winner_pid,None).await;
    let second=if first==1 {2} else {1};
    let waiter_task=tokio::spawn(async move {
        let mut request=input(second,1);
        if conflicting {request.system.label="Conflicting concurrent intent".to_owned();}
    ...
    assert_eq!(snapshot(connection).await,before,"uncommitted retry exposed partial state");
    let unlocked:bool=sqlx::query_scalar("SELECT pg_advisory_unlock(170017)").fetch_one(&mut *connection).await.unwrap();
    assert!(unlocked);
    assert_eq!(tokio::time::timeout(Duration::from_secs(5),winner_task).await.unwrap().unwrap().unwrap(),expected(first));
    let result=tokio::time::timeout(Duration::from_secs(5),waiter_task).await.unwrap().unwrap();
    if conflicting {assert!(matches!(result,Err(StorageError::Conflict)),"concurrent changed intent was not rejected");}
    else {assert_eq!(result.unwrap(),expected(first),"concurrent identical retry did not reuse original outcome");}
    assert_records(connection,&[first]).await;
    execute(connection,"DROP TRIGGER retry_wait ON public.outgoing_work").await;
    println!("Retry race passed: first-{first}-{}",if conflicting {"conflicting"} else {"identical"});
}

fn sequence(seed:u32)->Vec<u32> {
    let mut value=seed;
    (0..24).map(|_| {value=value.wrapping_mul(1_664_525).wrapping_add(1_013_904_223); (value>>16)%4}).collect()
}
...
async fn model(connection:&mut PgConnection) {
    ...
    for seed in [17,91,314] {
        ...
        for (index,operation) in operations.into_iter().enumerate() {
            ...
            let before=snapshot(connection).await;
            match operation {
                0 => assert_eq!(create_system_with_retry(connection,&input(candidate,current),Some(&key()), |_| true).await.unwrap(),expected(current)),
                1 => {
                    let mut request=input(candidate,current);
                    request.artifact.bytes=b"model changed content".to_vec();
                    assert!(matches!(create_system_with_retry(connection,&request,Some(&key()), |_| true).await,Err(StorageError::Conflict)));
                },
                2 => assert!(matches!(create_system_with_retry(connection,&input(candidate,current),Some(&key()), |_| false).await,Err(StorageError::Denied))),
                3 => {
                    execute(connection,"UPDATE public.system_create_retry SET retained_at=clock_timestamp()-interval '2 hours',expires_at=clock_timestamp()-interval '1 hour'").await;
                    assert_eq!(create_system_with_retry(connection,&input(candidate,candidate),Some(&key()), |_| true).await.unwrap(),expected(candidate));
                    current=candidate;
                    retained.push(candidate);
                },
                _ => unreachable!(),
            }
            if operation!=3 {assert_eq!(snapshot(connection).await,before,"model non-effect transition changed state");}
            assert_records(connection,&retained).await;
        }
    }
    assert!(partitions.iter().all(|count|*count>0),"generator missed a transition partition");
...
async fn serving(connection:&mut PgConnection) {
    ...
    assert_eq!(create_system_with_retry(&mut writer,&input(1,1),Some(&key()), |_| true).await.unwrap(),expected(1));
    assert_eq!(create_system_with_retry(&mut writer,&input(2,1),Some(&key()), |_| true).await.unwrap(),expected(1));
    assert_records(connection,&[1]).await;
    let before=snapshot(connection).await;
    assert!(matches!(create_system_with_retry(&mut writer,&input(2,2),Some(&key()), |_| true).await,Err(StorageError::Conflict)));
    assert_eq!(snapshot(connection).await,before);
    for sql in ["DELETE FROM public.system_create_retry", "UPDATE public.system_create_retry SET actor='other'"] {
    ...
    execute(connection,"UPDATE public.system_create_retry SET retained_at=clock_timestamp()-interval '2 hours',expires_at=clock_timestamp()-interval '1 hour'").await;
    assert_eq!(create_system_with_retry(&mut writer,&input(2,2),Some(&key()), |_| true).await.unwrap(),expected(2));
    assert_records(connection,&[1,2]).await;
...
async fn proof() {
    ...
    for (first,conflicting) in [(1,false),(2,false),(1,true),(2,true)] {concurrent(&mut connection,first,conflicting).await;}
...
fn main() {
    ...
    tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap().block_on(async {
        tokio::time::timeout(Duration::from_secs(90),proof()).await.expect("bounded retry write proof timed out");
    });
}
