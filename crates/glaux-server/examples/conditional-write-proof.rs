...
use glaux_server::storage::{StorageError, SystemRecord, SystemRepository, migrate, packaged_migrations};
...
fn id(suffix: u16) -> LocalId {
    format!("01890f20-7b5a-7cc3-98c4-dc0c0c08{suffix:04}").parse().unwrap()
}
...
fn label(number: u16) -> &'static str {
    match number { 1 => "Original", 2 => "Writer A", 3 => "Writer B", _ => unreachable!() }
}

fn bytes(number: u16) -> &'static [u8] {
    match number { 1 => b"original-bytes", 2 => b"writer-A-bytes", 3 => b"writer-B-bytes", _ => unreachable!() }
}
...
fn update(number: u16, expected: Option<u16>) -> UpdateSystem {
    UpdateSystem {
        ...
        revision: SystemRevision {
            id: revision(number), system_id: id(102),
            artifact_id: id(200 + number).to_string().parse().unwrap(),
            semantic_time: None, receipt_time: TIME.parse().unwrap(),
        },
        ...
        audit: AuditContext {
            ...
            correlation: format!("conditional-{number}"), time: TIME.parse().unwrap(),
        },
...
fn parent() -> SystemRecord {
    SystemRecord { id: id(101), uid: "urn:glaux:test:conditional-parent".parse().unwrap(),
        label: "Parent".to_owned(), parent: None, sources: vec![] }
}
...
fn original() -> CreateSystem {
    ...
    CreateSystem {
        system: SystemRecord {
            id: id(102), uid: "urn:glaux:test:conditional-child".parse().unwrap(),
            label: "Original".to_owned(), parent: Some(id(101)),
            sources: vec![SourceIdentity::new("urn:glaux:test:conditional-source".parse().unwrap(),
                                             "upstream-child".parse().unwrap())],
        },
        artifact: input.artifact, revision: input.revision,
        audit_id: input.audit_id, event_id: input.event_id, audit: input.audit,
    }
}
...
fn passed(group: &str) { println!("Conditional write group passed: {group}"); }
...
async fn assert_facts(connection: &mut PgConnection, current: u16, history: &[u16]) {
    ...
    assert_eq!(identity, (id(102).to_string(), "urn:glaux:test:conditional-child".to_owned(),
        label(current).to_owned(), id(101).to_string(),
        "urn:glaux:test:conditional-source".to_owned(), "upstream-child".to_owned()));
    ...
    assert_eq!(head, vec![(id(102).to_string(), id(300 + current).to_string(), id(200 + current).to_string())]);
    for (table, base) in [("source_artifact", 200), ("system_revision", 300), ("server_audit", 400), ("outgoing_work", 500)] {
        let ids: Vec<String> = sqlx::query_scalar(&format!("SELECT id::text FROM public.{table} ORDER BY id"))
            .fetch_all(&mut *connection).await.unwrap();
        assert_eq!(ids, history.iter().map(|number| id(base + number).to_string()).collect::<Vec<_>>(), "exact retained set for {table}");
    }
    for number in history {
        ...
        for (column, expected) in [
            ("system_id", id(102).to_string()), ("artifact_id", id(200 + number).to_string()),
            ("receipt_fraction", "0.12345678901234567890".to_owned()), ("receipt_source", TIME.to_owned()),
            ("media_type", "application/octet-stream".to_owned()), ("actor", "conditional-actor".to_owned()),
            ("source", "conditional-source".to_owned()), ("target_id", id(102).to_string()),
            ("revision_id", id(300 + number).to_string()), ("time_fraction", "0.12345678901234567890".to_owned()),
            ("time_source", TIME.to_owned()), ("outcome", "accepted".to_owned()), ("correlation", format!("conditional-{number}")),
            ("work_system", id(102).to_string()), ("work_revision", id(300 + number).to_string()),
            ("work_artifact", id(200 + number).to_string()), ("work_audit", id(400 + number).to_string()),
            ("work_outcome", "accepted".to_owned()),
        ] { assert_eq!(row.get::<String, _>(column), expected, "retained {number} {column}"); }
        let operation = if *number == 1 { "system.create" } else { "system.update" };
        assert_eq!(row.get::<String, _>("operation"), operation);
        assert_eq!(row.get::<String, _>("audit_operation"), operation);
        assert_eq!(row.get::<String, _>("kind"), if *number == 1 { "system.created" } else { "system.updated" });
        assert_eq!(row.get::<Vec<u8>, _>("bytes"), bytes(*number));
        assert!(row.get::<Option<String>, _>("semantic_source").is_none());
        for column in ["receipt_civil_second", "time_civil_second"] { assert_eq!(row.get::<i64, _>(column), 1_483_228_800); }
        for column in ["receipt_leap", "time_leap"] { assert!(!row.get::<bool, _>(column)); }
    }
}

async fn migration(connection: &mut PgConnection) {
    packaged_migrations().run_to(6, &mut *connection).await.unwrap();
    ...
    for number in [1, 3] {
        ...
        sqlx::query("INSERT INTO public.system_revision VALUES($1::text::uuid,$2::text::uuid,$3::text::uuid,
                    NULL,NULL,NULL,NULL,$4,false,0,'2030-01-01T00:00:00Z')")
            .bind(id(300 + number).to_string()).bind(id(102).to_string()).bind(id(200 + number).to_string())
            .bind(1_893_456_000_i64 + i64::from(number)).execute(&mut *connection).await.unwrap();
    ...
    assert!(migrate(connection).await.is_err(), "ambiguous migration must fail, not choose a head");
    ...
    assert_eq!(snapshot(connection, false).await, before, "failed migration mutated schema-6 data");
    let versions: Vec<i64> = sqlx::query_scalar("SELECT version FROM public._sqlx_migrations ORDER BY version")
        .fetch_all(&mut *connection).await.unwrap();
    ...
    let head: (String, String) = sqlx::query_as("SELECT revision_id::text,artifact_id::text FROM public.system_write_head")
        .fetch_one(&mut *connection).await.unwrap();
    assert_eq!(head, (id(301).to_string(), id(201).to_string()), "migration selected newest unattached history");
    let retained: Vec<String> = sqlx::query_scalar("SELECT id::text FROM public.system_revision ORDER BY id")
        .fetch_all(&mut *connection).await.unwrap();
...
async fn sequential(connection: &mut PgConnection) {
    ...
    let receipt = update_system(connection, &update(2, Some(1))).await.unwrap();
    ...
    let before = snapshot(connection, true).await;
    assert!(matches!(update_system(connection, &update(3, Some(1))).await,
                     Err(StorageError::PreconditionFailed)), "stale supplied condition was not rejected");
    assert_eq!(snapshot(connection, true).await, before, "stale rejection changed state");
    ...
    assert_eq!(snapshot(connection, true).await, before);
    migrate(connection).await.unwrap();
    assert_eq!(snapshot(connection, true).await, before, "migration reapply rewound an advanced head");
...
async fn invalid(connection: &mut PgConnection) {
    ...
    for (target, missing) in [(id(199), true), (id(101), false)] {
        ...
        assert!(if missing { matches!(error, StorageError::NotFound) } else { matches!(error, StorageError::UninitializedRevision) });
        assert_eq!(snapshot(connection, true).await, before);
    }
    for case in 0..6 {
        ...
        assert!(matches!(update_system(connection, &input).await, Err(StorageError::InvalidInput)), "invalid case {case}");
        assert_eq!(snapshot(connection, true).await, before);
    }
    let mut transaction = connection.begin().await.unwrap();
    assert!(matches!(update_system(&mut transaction, &update(2, Some(1))).await, Err(StorageError::InvalidInput)));
...
async fn rollback(connection: &mut PgConnection) {
    execute(connection, "CREATE FUNCTION public.conditional_failure() RETURNS trigger LANGUAGE plpgsql AS $$
        BEGIN RAISE EXCEPTION 'conditional fixture boundary' USING ERRCODE='P0001'; END; $$").await;
    for (name, table, action, deferred) in [
        ...
        let error = update_system(connection, &update(2, Some(1))).await.unwrap_err();
        assert!(matches!(&error, StorageError::Database(inner) if inner.as_database_error().and_then(|e| e.code()).as_deref()==Some("P0001")), "wrong rollback failure {name}: {error}");
        execute(connection, &format!("DROP TRIGGER conditional_fail ON public.{table}")).await;
        assert_eq!(snapshot(connection, true).await, before, "partial update after {name}");
        assert!(!connection.is_in_transaction());
...
async fn concurrent(connection: &mut PgConnection, first: u16, conditional: bool) {
    ...
    let winner_task = tokio::spawn(async move { update_system(&mut winner, &update(first, Some(1))).await });
    wait_for_lock(connection, winner_pid, None).await;
    let second = if first == 2 { 3 } else { 2 };
    let waiter_task = tokio::spawn(async move { update_system(&mut waiter, &update(second, if conditional { Some(1) } else { None })).await });
    wait_for_lock(connection, waiter_pid, Some(winner_pid)).await;
    assert!(!winner_task.is_finished() && !waiter_task.is_finished());
    assert_eq!(snapshot(connection, true).await, before, "uncommitted update became visible");
    let unlocked: bool = sqlx::query_scalar("SELECT pg_advisory_unlock(160016)").fetch_one(&mut *connection).await.unwrap();
    assert!(unlocked);
    let winner_receipt = tokio::time::timeout(Duration::from_secs(5), winner_task).await.unwrap().unwrap().unwrap();
    ...
    if conditional {
        assert!(matches!(result, Err(StorageError::PreconditionFailed)), "conditional waiter did not reject stale head");
    ...
    execute(connection, "DROP TRIGGER conditional_wait ON public.outgoing_work").await;
    println!("Conditional race passed: first-{first}-{}", if conditional { "conditional" } else { "unconditional" });
}
...
async fn serving(connection: &mut PgConnection) {
    ...
    update_system(&mut writer, &update(2, Some(1))).await.unwrap();
    assert_facts(connection, 2, &[1, 2]).await;
    let before = snapshot(connection, true).await;
    assert!(matches!(update_system(&mut writer, &update(3, Some(1))).await, Err(StorageError::PreconditionFailed)));
    assert_eq!(snapshot(connection, true).await, before);
...
fn main() {
    ...
    tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap().block_on(async {
        tokio::time::timeout(Duration::from_secs(90), proof()).await.expect("bounded conditional write proof timed out");
    });
}
