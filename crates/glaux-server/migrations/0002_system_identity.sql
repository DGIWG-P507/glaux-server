...
CREATE TABLE system_identity (
    id uuid PRIMARY KEY REFERENCES resource_identity(id),
    label text NOT NULL
);

CREATE TABLE source_identity (
    resource_id uuid NOT NULL REFERENCES resource_identity(id),
...
CREATE INDEX source_identity_resource ON source_identity(resource_id);
