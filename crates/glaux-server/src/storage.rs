...
pub fn packaged_migrations() -> Migrator {
    Migrator::with_migrations(vec![
        Migration::new(1, "enable PostGIS".into(), MigrationType::Simple,
            include_str!("../migrations/0001_enable_postgis.sql").into_sql_str(), false),
        Migration::new(2, "System identity".into(), MigrationType::Simple,
            include_str!("../migrations/0002_system_identity.sql").into_sql_str(), false),
        Migration::new(3, "System parent".into(), MigrationType::Simple,
            include_str!("../migrations/0003_system_parent.sql").into_sql_str(), false),
    ])
}
...
pub async fn migrate(connection: &mut PgConnection) -> Result<(), StorageError> {
    packaged_migrations().run(connection).await.map_err(StorageError::Migration)
}
...
pub async fn check_schema(connection: &mut PgConnection) -> Result<(), StorageError> {
    ...
    if !exists { return Err(StorageError::IncompatibleSchema); }
    let rows = sqlx::query("SELECT version, checksum, success FROM public._sqlx_migrations ORDER BY version")
        .fetch_all(&mut *connection).await?;
    let migrations = packaged_migrations();
    if rows.len() != migrations.migrations.len() { return Err(StorageError::IncompatibleSchema); }
...
impl SystemRepository {
    ...
    pub async fn create(connection: &mut PgConnection, record: &SystemRecord) -> Result<(), StorageError> {
        ...
        match result {
            Ok(()) => { transaction.commit().await?; Ok(()) }
            Err(error) => { transaction.rollback().await?; Err(error.into()) }
        }
    }

    /// All constituent rows are read by one statement, including zero/multiple aliases.
    pub async fn get(connection: &mut PgConnection, id: LocalId) -> Result<Option<SystemRecord>, StorageError> {
        check_schema(connection).await?;
        let rows = sqlx::query(
        ...
        let mut record = SystemRecord {
            id: first.try_get::<String, _>("id")?.parse().map_err(|_| StorageError::InvalidStoredValue)?,
            uid: first.try_get::<String, _>("uid")?.parse().map_err(|_| StorageError::InvalidStoredValue)?,
            label: first.try_get("label")?,
            parent: first.try_get::<Option<String>, _>("parent")?.map(|value| value.parse())
                .transpose().map_err(|_| StorageError::InvalidStoredValue)?,
            sources: Vec::new(),
        };
        for row in rows {
            if let Some(authority) = row.try_get::<Option<String>, _>("authority")? {
                record.sources.push(SourceIdentity::new(
                    authority.parse().map_err(|_| StorageError::InvalidStoredValue)?,
                    row.try_get::<String, _>("identifier")?.parse().map_err(|_| StorageError::InvalidStoredValue)?,
                ));
            }
        }
        Ok(Some(record))
    ...
    pub async fn find_uid(connection: &mut PgConnection, uid: &Uid) -> Result<Option<SystemRecord>, StorageError> {
        check_schema(connection).await?;
        let id: Option<String> = sqlx::query_scalar("SELECT id::text FROM public.resource_identity WHERE uid=$1 COLLATE \"C\"")
            .bind(uid.as_str()).fetch_optional(&mut *connection).await?;
        match id {
            Some(id) => Self::get(connection, id.parse().map_err(|_| StorageError::InvalidStoredValue)?).await,
            None => Ok(None),
        }
    }

    pub async fn find_source(connection: &mut PgConnection, source: &SourceIdentity) -> Result<Option<SystemRecord>, StorageError> {
        check_schema(connection).await?;
        ...
        ).bind(source.authority().as_str()).bind(source.identifier().as_str()).fetch_optional(&mut *connection).await?;
        match id {
            Some(id) => Self::get(connection, id.parse().map_err(|_| StorageError::InvalidStoredValue)?).await,
            None => Ok(None),
        }
