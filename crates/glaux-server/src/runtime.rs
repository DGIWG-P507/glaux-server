...
impl Health {
    async fn storage_ready(&self) -> bool {
        matches!(tokio::time::timeout(self.timeout, async {
...
async fn live() -> Response { response(StatusCode::OK, "alive\n") }
...
pub enum RuntimeError { Storage, Listener, Signal, Serving, Shutdown }
...
pub async fn serve(config: Configuration) -> Result<(), RuntimeError> {
    ...
    let pool = PgPoolOptions::new().max_connections(2).min_connections(0)
        .acquire_timeout(config.timeout()).connect_lazy_with(config.database());
    let health = Health { pool: pool.clone(), timeout: config.timeout() };
    if !health.storage_ready().await {
        pool.close().await;
        return Err(RuntimeError::Storage);
    }
    let listener = TcpListener::bind(config.listener()).await.map_err(|_| RuntimeError::Listener)?;
    let app = Router::new().route("/health/live", get(live)).route("/health/ready", get(ready)).with_state(health);
    ...
    let signal = async move {
        ...
        #[cfg(not(unix))]
        { tokio::signal::ctrl_c().await.map_err(|_| RuntimeError::Signal) }
    };
    let mut server = tokio::spawn(async move {
        axum::serve(listener, app).with_graceful_shutdown(async { let _ = shutdown_rx.await; }).await
    });
    println!("Health listener ready.");
    let result = tokio::select! {
        result = &mut server => result.map_err(|_| RuntimeError::Serving)?.map_err(|_| RuntimeError::Serving),
        signal_result = signal => {
            let _ = shutdown_tx.send(());
            match tokio::time::timeout(Duration::from_secs(5), &mut server).await {
                Ok(result) => { result.map_err(|_| RuntimeError::Serving)?.map_err(|_| RuntimeError::Serving)?; signal_result }
                Err(_) => { server.abort(); let _ = server.await; Err(RuntimeError::Shutdown) }
            }
        }
    };
    pool.close().await;
