...
fn unfinished_server_does_not_report_success() {
    ...
    assert_eq!(output.status.code(), Some(2), "unfinished startup must fail");
    assert!(output.stdout.is_empty(), "no successful response is available");
    assert_eq!(
        output.stderr,
        b"glaux-server: bootstrap only; no server commands are implemented yet.\n",
