...
impl ExactInstant {
    pub fn parse_rfc3339(text: &str) -> Result<Self, TimeError> {
        if text.len() > MAX_TIMESTAMP_BYTES {
            return Err(TimeError::InputLimit);
        }
        ...
        if leap
            && !POSITIVE_LEAP_DATES
                .iter()
                .any(|&(y, m, d)| civil_days(y, m, d) * 86_400 + 86_399 == civil_second)
        {
            return Err(TimeError::UnrecognizedLeapSecond);
        }
        let fraction_text = if fraction_digits == 0 {
            "0".to_owned()
        } else {
            format!("0.{}", &text[fraction_start..pos])
        };
        let fraction =
            ExactNumber::parse_json_number(&fraction_text).map_err(|_| TimeError::Syntax)?;
        Ok(Self {
            civil_second,
            leap,
            fraction,
    ...
    pub fn from_storage_parts(
        ...
        let stored_fraction =
            ExactNumber::parse_json_number(fraction).map_err(|_| TimeError::InconsistentStorage)?;
        if civil_second != parsed.civil_second
            || leap != parsed.leap
    ...
    pub fn civil_second(&self) -> i64 {
        self.civil_second
    }
    pub fn is_leap_second(&self) -> bool {
        self.leap
    }
    pub fn fraction(&self) -> &ExactNumber {
        &self.fraction
    }
    ...
    pub fn fraction_decimal(&self) -> String {
        if self.fraction_digits == 0 {
            ...
            format!(
                "0.{}",
                &self.source[self.fraction_start..self.fraction_start + self.fraction_digits]
            )
        }
    }
    pub fn source_lexeme(&self) -> &str {
        &self.source
    }
...
mod tests {
    ...
    fn instant(text: &str) -> ExactInstant {
        text.parse().unwrap()
    }
    ...
    fn calendar_and_epoch_values_are_independent() {
        ...
        for source in [
            ...
            assert_eq!(
                ExactInstant::parse_rfc3339(source),
                Err(TimeError::Calendar)
            );
        }
    }
    ...
    fn offset_sign_and_unknown_offset_preserve_instant() {
        ...
        assert_eq!(
            instant("1996-12-19T16:39:57-08:00"),
            instant("1996-12-20T00:39:57Z")
        );
        assert_eq!(
            instant("2000-03-01T00:00:00+00:01"),
            instant("2000-02-29T23:59:00Z")
        );
    ...
    fn fractional_boundaries_are_not_rounded() {
        ...
        assert_eq!(
            first.fraction(),
            &"0.1234561".parse::<ExactNumber>().unwrap()
        );
        assert_eq!(first.fraction_decimal(), "0.1234561");
        assert!(
            instant("1969-12-31T23:59:59.9999999999999999999Z") < instant("1970-01-01T00:00:00Z")
        );
        assert!(
            instant("1970-01-01T00:00:00.0000000000000000001Z") > instant("1970-01-01T00:00:00Z")
        );
        let variants = [
            instant("1970-01-01T00:00:00.1Z"),
            instant("1970-01-01t01:00:00.100+01:00"),
        ];
    ...
    fn leap_slot_is_distinct_and_offsets_are_checked_in_utc() {
        ...
        assert_eq!(
            instant("1990-12-31T23:59:60Z"),
            instant("1990-12-31T15:59:60-08:00")
        );
        ...
        for source in [
            ...
            assert_eq!(
                ExactInstant::parse_rfc3339(source),
                Err(TimeError::UnrecognizedLeapSecond)
            );
        }
    ...
    fn syntax_and_input_budgets_fail_without_repair() {
        ...
        let boundary = format!(
            "1970-01-01T00:00:00.{}1Z",
            "0".repeat(MAX_TIMESTAMP_BYTES - 22)
        );
        ...
        assert_eq!(
            ExactInstant::parse_rfc3339(&format!("{boundary}Z")),
            Err(TimeError::InputLimit)
        );
    }
    ...
    fn storage_parts_must_match_source_without_normalizing_away_errors() {
        ...
        let restored =
            ExactInstant::from_storage_parts(0, false, "0.123456789012345678900", source).unwrap();
        assert_eq!(restored, parsed);
        assert_eq!(restored.source_lexeme(), source);
        assert_eq!(restored.fraction_digits(), 19);
        for (second, leap, fraction) in [
            (1, false, "0.1234567890123456789"),
            (0, true, "0.1234567890123456789"),
            (0, false, "0.123457"),
            (0, false, "NaN"),
            (0, false, "-0.1"),
            (0, false, "1"),
            ...
            assert_eq!(
                ...
                Err(TimeError::InconsistentStorage)
            );
        }
    }
}
