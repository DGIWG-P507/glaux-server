...
impl ExactNumber {
    ...
    pub fn parse_json_number(text: &str) -> Result<Self, NumericError> {
        ...
        if bytes.get(pos) == Some(&b'.') {
            ...
            if fraction_digits == 0 {
                return Err(NumericError::Syntax);
            }
        }
        ...
        let digits: Vec<u8> = bytes[start..mantissa_end]
            .iter()
            .copied()
            .filter(|b| *b != b'.')
            .collect();
        ...
        Ok(Self {
            value,
            source: Source::Decimal(text.to_owned()),
        })
    }
    ...
    pub fn decimal_lexeme(&self) -> Option<&str> {
        match &self.source {
            Source::Decimal(text) => Some(text),
            Source::Binary64(_) => None,
        }
    }
    pub fn binary64_bits(&self) -> Option<u64> {
        match self.source {
            Source::Binary64(bits) => Some(bits),
            Source::Decimal(_) => None,
        }
    }
...
impl TryFrom<ExactNumber> for CountValue {
    ...
    fn try_from(value: ExactNumber) -> Result<Self, Self::Error> {
        if !value.is_integer() {
            return Err(NumericError::NonIntegral);
        }
        Ok(Self(value))
    }
}
impl FromStr for CountValue {
    type Err = NumericError;
    fn from_str(text: &str) -> Result<Self, Self::Err> {
        text.parse::<ExactNumber>()?.try_into()
    }
}
impl CountValue {
...
impl NumericValue {
    ...
    pub fn from_binary64(value: f64) -> Self {
        if value.is_nan() {
            ...
            Self::Finite(
                ExactNumber::from_binary64(value).expect("finite binary64 has an exact ratio"),
            )
        }
    }
}
...
mod tests {
    ...
    fn distinct_large_integers_never_collapse() {
        ...
        assert_eq!(
            number("18446744073709551616").cmp(&number("18446744073709551617")),
            Ordering::Less
        );
        assert_eq!(
            number("-18446744073709551617").cmp(&number("-18446744073709551616")),
            Ordering::Less
        );
    }
    ...
    fn decimal_fractions_exponents_and_value_hashing_are_exact() {
        ...
        assert_eq!(
            number("0.1000000000000000000000000001").cmp(&tenth),
            Ordering::Greater
        );
        assert_eq!(
            number("-0.1000000000000000000000000001").cmp(&number("-0.1")),
            Ordering::Less
        );
        assert_eq!(
            [number("1"), number("1.0"), number("10e-1")]
                .into_iter()
                .collect::<HashSet<_>>()
                .len(),
            1
    ...
    fn count_integrality_and_requested_integer_ranges_are_checked() {
        ...
        assert_eq!(
            "-1e-400".parse::<CountValue>(),
            Err(NumericError::NonIntegral)
        );
        assert_eq!(
            "-9223372036854775808"
                .parse::<CountValue>()
        ...
        assert_eq!(
            "-1".parse::<CountValue>().unwrap().try_to_u64(),
            Err(NumericError::OutOfRange)
        );
    ...
    fn json_token_grammar_rejects_repairs_and_special_numbers() {
        for text in [
            ...
            assert_eq!(
                ExactNumber::parse_json_number(text),
                Err(NumericError::Syntax),
                "{text:?}"
            );
        }
    }
    ...
    fn numeric_budgets_fail_explicitly_before_expansion() {
        ...
        assert_eq!(
            ExactNumber::parse_json_number(&format!("{at_limit}9")),
            Err(NumericError::InputLimit)
        );
        assert!(number("1e4096") > number("1e4095"));
        assert!(number("1e-4096") > number("0"));
        assert_eq!(
            ExactNumber::parse_json_number("1e4097"),
            Err(NumericError::ExponentLimit)
        );
        assert_eq!(
            ExactNumber::parse_json_number("1e-4097"),
            Err(NumericError::ExponentLimit)
        );
        assert_eq!(
            ExactNumber::parse_json_number("0e999999999999999999999"),
            Err(NumericError::ExponentLimit)
        );
    ...
    fn nonfinite_states_are_explicit_and_nan_is_unordered() {
        ...
        for text in ["Infinity", "+Infinity"] {
            assert_eq!(
                NumericValue::from_swe_special(text).unwrap(),
                PositiveInfinity
            );
        }
        assert_eq!(
            NumericValue::from_swe_special("-Infinity").unwrap(),
            NegativeInfinity
        );
        for text in ["INF", "-INF", "nan", "1", "null", " NaN", "\"NaN\""] {
            assert_eq!(
                NumericValue::from_swe_special(text),
                Err(NumericError::Syntax)
            );
        }
        ...
        assert_eq!(
            NumericValue::from_binary64(f64::NEG_INFINITY),
            NegativeInfinity
        );
        for value in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
            assert_eq!(
                ExactNumber::from_binary64(value),
