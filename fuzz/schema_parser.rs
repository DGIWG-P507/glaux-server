...
use glaux_standards::validation::{Contract, Failure, StructuralValidator, MAX_BYTES, MAX_DEPTH};
...
fn main() {
    ...
    for index in 0..1024 {
        ...
        if index % 4 != 0 {
            ...
            match index % 3 {
                0 => { bytes[at] ^= (state >> 32) as u8; }
                1 => { bytes.truncate(at); }
                _ => { bytes.insert(at, (state >> 24) as u8); }
            }
        }
        let result = validator.validate(Contract::Quantity, &bytes);
        assert_eq!(result, validator.validate(Contract::Quantity, &bytes), "nondeterministic input {bytes:?}");
        if result.is_ok() {
            accepted += 1;
    ...
    assert!(accepted >= 256 && rejected > 0, "mutation partitions did not execute");
    assert_eq!(validator.validate(Contract::Quantity, &vec![b' '; MAX_BYTES + 1]), Err(Failure::Size));
    assert_eq!(validator.validate(Contract::Quantity, &vec![b'['; MAX_DEPTH + 1]), Err(Failure::Depth));
    println!("Schema-parser fuzz v1: 1024 cases; seed=0x475c000800000001; accepted={accepted}; rejected={rejected}; deterministic verdict, required fields and explicit bounds checked.");
}
