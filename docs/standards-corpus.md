...
The manifest defines the fixed URI-to-file allowlist. Relative references use the original source base and must reach a packaged schema; fragments must identify a local target. A URI outside this mapping must fail instead of falling back to the network or an arbitrary filesystem path. URI and path spelling remain significant. This lookup is corpus inspection infrastructure; public-request resolution and validator behavior remain work for issue #8 and its consumers.
...
- Recursive SWE records/arrays and recursive SensorML components/outputs have positive inputs and deep negative variants. Their finite shapes exercise recursive schema references; depth limits, cycle handling and complete component semantics still require later implementation and tests.
